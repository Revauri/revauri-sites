// Fetches real available times for the Revauri AI call from the Calendly
...
const EVENT_SCHEDULING_URL = "https://calendly.com/joseph-revauri/revauri-ai-call";
...
async function resolveEventTypeUri(token: string): Promise<string> {
  ...
  if (!match?.uri) {
    ...
    throw new Error(
      `Revauri AI call event type not found on Calendly account. Expected ${EVENT_SCHEDULING_URL}, ` +
