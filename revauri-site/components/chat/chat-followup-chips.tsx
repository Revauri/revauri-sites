...
// booking or lead card is its own CTA (chips underneath would be noise), while
// portfolio cards pair with the two conversion paths. Anything else gets the
// default trio — the same prompts as the empty-state starters.
export function followupsFor(message: UIMessage): string[] {
  ...
  for (const part of message.parts) {
    ...
    if (part.type === "tool-show_portfolio" && part.state === "output-available") {
      hasPortfolio = true;
    }
  }
  if (hasCardCta) return [];
  if (hasPortfolio) return ["Book a call", "Get pricing"];
  return ["Get pricing", "Book a call", "View portfolio"];
}
