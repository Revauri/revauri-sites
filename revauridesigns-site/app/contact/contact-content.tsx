...
const CONTACT_EMAIL = "ryan.calloway@revauridesigns.com";
...
export function ContactContent({ isSubmitted = false }: ContactContentProps) {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = [
      ...
      `How did you hear about us: ${String(data.get("referral") ?? "")}`,
      "",
      String(data.get("message") ?? ""),
    ].join("\n");
    const href = `mailto:${CONTACT_EMAIL}?subject=${encodeURIComponent("New message from revauridesigns.com")}&body=${encodeURIComponent(body)}`;
        ...
        <FadeInWhenVisible className="lg:sticky lg:top-28">
          ...
          <ul className="mt-8 space-y-4 text-sm">
            <ContactMethod
              ...
              label="ryan.calloway@revauridesigns.com"
            />
            <ContactMethod
              href="/book"
              icon={<Phone className="h-4 w-4" aria-hidden="true" />}
              label="Book a 15-minute strategy call"
              internal
            />
            <ContactMethod
